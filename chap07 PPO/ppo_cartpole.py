...
Experience = collections.namedtuple(typename="Experience",
                                    field_names=['state', 'action', 'reward', 'done', 'nextState', 'action_prob'])
...
class Agent(object):
    ...
    def choose_action(self, state):
        ...
        action = c.sample()
        return action, action_prob[:, action.item()].item()
    ...
    def learn(self):
        ...
        old_action_prob = torch.unsqueeze(torch.FloatTensor(old_action_prob), 1)

        state_v = self.critic(states_tensor)
        delta = rewards_tensor - state_v
        advantage = delta.detach()
        new_action_prob = self.actor(states_tensor).gather(1, actions_tensor)
        ...
        # Compute critic loss
        critic_loss = F.mse_loss(rewards_tensor, state_v)
...
def main():
    ...
    for epoch in range(10000):
        ...
        while not done:
            ...
            agent.store_transition(state, action.item(), r, done, state_next, action_prob)
            if not done:
                state = state_next
            episode_r.append(r)
        agent.learn()
