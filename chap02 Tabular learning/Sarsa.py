...
class OptiFunctionEnv(gym.Env):
    ...
    def step(self, action):
        ...
        done = bool(self.state < -self.length or self.state > self.length or self.count >= 200)

        if done:
            reward = -(np.power(self.state, 2) - np.power(pre_state, 2))
        else:
            reward = -(np.power(self.state, 2) - np.power(pre_state, 2))
    ...
    def render(self, mode='human'):
        view = ['-']*(self.length*2 + 2)
        view[self.state+1+self.length] = '*'
        view = ''.join(view)
        time.sleep(0.1)
        print(view)
...
class Agent(object):
    ...
    def learn(self):
        ...
        if not done:
            next_action = self.choose_action(next_state)  # sarsa update method
        else:
            next_action = action
        ...
        if done:
            ...
            delta = r + self.args.gamma * self.q_table.iloc[next_state, next_action] - self.q_table.loc[state, action]
...
def main():
    ...
    for epoch in range(10000):
        ...
        while not done:
            ...
            if not done: state = state_next  # 5. 更新状态
            episode_r.append(r)
