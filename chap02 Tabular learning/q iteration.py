...
class Agent:
    ...
    def Update_Q_value(self, state, action):
        ...
        for next_state, count in next_state_all.items():
            ...
            # Q(s,a) = \sum{P(s^{\prime}|s,a)(r + \gamma * Q(s^{\prime},argmax_{a}Q(s^{\prime},a)))}
            best_action = self.selection_action(next_state)
            action_value += (count/total_count) * (r + args.gamma*self.values[(next_state, best_action)])
